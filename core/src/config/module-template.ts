...
export async function resolveTemplatedModule(
  ...
  // when resolving the resolving the resulting modules. Inputs that are used module names must however be resolvable
  // immediately.
  ...
  const modules = await Bluebird.map(template.modules || [], async (m) => {
    ...
    try {
      ...
      if (spec.name && spec.name.includes && spec.name.includes("${")) {
        msg +=
          ". Note that if a template string is used in the name of a module in a template, that the template string must be fully resolvable at the time of module scanning. This means that e.g. references to other modules or runtime outputs cannot be used."
      }
